...
func init() {
	f := mmapOpenFile(os.O_RDWR | os.O_CREATE | os.O_TRUNC)
	f.Write(testData)
	f.Close()
}

func mmapOpenFile(flags int) *os.File {
...
func TestUnmap(t *testing.T) {
	f := mmapOpenFile(os.O_RDONLY)
	defer f.Close()
	mmap, err := MapFile(f, MMAP_RO, 0)
	if err != nil {
...
func TestReadWrite(t *testing.T) {
	f := mmapOpenFile(os.O_RDWR)
	defer f.Close()
	mmap, err := MapFile(f, MMAP_RW, 0)
	if err != nil {
		t.Errorf("error mapping: %s", err)
	}
	defer mmap.Unmap()
...
func TestProtFlagsAndErr(t *testing.T) {
	f := mmapOpenFile(os.O_RDONLY)
	defer f.Close()
	if _, err := MapFile(f, MMAP_RW, 0); err == nil {
...
func TestFlags(t *testing.T) {
	f := mmapOpenFile(os.O_RDWR)
	defer f.Close()
	mmap, err := MapFile(f, MMAP_COPY, 0)
	if err != nil {
		t.Errorf("error mapping: %s", err)
	}
	defer mmap.Unmap()
...
func TestNonZeroOffset(t *testing.T) {
	...
	m, err := MapFileEx(fileobj, pageSize, MMAP_RO, 0, 0)
	if err != nil {
	...
	m, err = MapFileEx(fileobj, pageSize, MMAP_RO, 0, pageSize)
	if err != nil {
		t.Errorf("error mapping file: %s", err)
	}
	err = m.Unmap()
	...
	m, err = MapFileEx(fileobj, pageSize, MMAP_RO, 0, 1)
	if err == nil {
